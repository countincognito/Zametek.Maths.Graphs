...
- **P6 and P7, the small allocation items - done, and worth 0.05%.** `StartTime` and `FinishTime` on both builders were LINQ chains allocating an iterator and a closure per access; they are explicit loops now. `ResourceScheduleBuilder.ScheduledActivities` copied the whole schedule on every read, and the only place in the library that did so wanted a single number out of it - the earliest start time - so there is now a `FirstActivityStartTime` property alongside the existing `LastActivityFinishTime`, and the scheduler reads that. `Compile` materialised `Activities.ToList()` twice in adjacent statements that cannot change the collection between them, and now materialises it once.

  Measured deterministically, because these are far too small to see through wall-clock noise: at the activity limit a compile allocates **73,853 KB against 73,888 KB before, a saving of 35 KB, or 0.05%**. That is exactly what the changes should save and no more - one list of 2,000 references is 16 KB, the schedule copies another 16 KB, the iterators the rest - so the accounting closes, and the honest summary is that they were correct to do and made no difference. The reason to record the figure is that P6 and P7 sat on the outstanding list for a long time reading like latent wins.

  Two things are deliberately not done here. `ActivityIds` stays as `Activities.Select(x => x.Id)`, because it returns `IEnumerable<T>` by contract and materialising it would allocate more, not less. And `ScheduledActivities` still returns a snapshot: with the scheduler reading `FirstActivityStartTime` instead, nothing inside the library calls it at all, and for an external consumer a snapshot is the right semantics - handing out a live view of the builder's own list would let a later append change a collection the caller is still holding. `ResourceScheduleBuilderTests` pins the ordering property the substitution rests on, and was checked against a seeded fault; notably the existing scheduler tests did not catch that fault, so the new tests are carrying it.

  Checked against the Rust port afterwards, which split three ways and is worth recording because it is the pattern this work keeps running into. **P7 did not exist there**: `scheduled_activities()` returns a borrowed slice, so the port never had the copy - the third time it has turned out to be the correct side. **P6 has no counterpart**: Rust's iterator chains compile to the same loop the C# was rewritten into, with no per-access allocation to remove, so the C# change closed a gap rather than opening one. **The compiler duplication was real and is now fixed there too** - `activity_ids()` was collected twice in adjacent statements, shadowed rather than shared. The `first_activity_start_time` accessor was ported as well, though it saves nothing there, because the two implementations being readable side by side is what makes divergences like these findable at all.

  One difference is deliberate and should stay. Rust caches activity **IDs** where C# caches activity **references**, so sharing a collection across a wider span is a weaker claim there than here: an ID re-resolves against current state, a reference can go stale. Both languages therefore share only across the two adjacent calls, and both leave the earlier `ResetResourceState` collection separate.
...
| Activities | Depth | Compile | Priority list (incremental) | Full CPM passes | Scheduling tick loop | Everything else | Allocated |
| - | - | - | - | - | - | - | - |
| 250 | 10 | 46 ms | 29 ms | 1 ms | 3 ms | 13 ms | 4.4 MB |
| 500 | 20 | 45 ms | 21 ms | 1 ms | 4 ms | 18 ms | 10.4 MB |
| 1,000 | 40 | 136 ms | 86 ms | 1 ms | 6 ms | 42 ms | 25.1 MB |
| 1,500 | 60 | 238 ms | 164 ms | 5 ms | 11 ms | 58 ms | 46.0 MB |
| 2,000 | 80 | 425 ms | 301 ms | 2 ms | 23 ms | 99 ms | 72.1 MB |

**Read the times as proportions, not as figures.** Repeated runs of the same build at 2,000 activities gave totals between 357 ms and 476 ms - a spread of a quarter, even taking the best of five - while allocation repeated to within a few hundred bytes. The split between the columns is stable across those runs; the absolute numbers are not. Any change too small to move a proportion should be evaluated on allocation instead, which is how the P6 and P7 items above were settled.

Three things follow.

**A compile at the activity limit now takes something in the region of 0.4 seconds**, against 6.9 seconds when this investigation began. The aligned-rebuild and indirect-schedule stages are under a millisecond throughout and are omitted from the table.

**The scheduler is not the cost - 23 ms of 425 ms, around 5%.** Varying the resource count at 2,000 activities isolates `ActivityAt` directly, since its scan covers one resource's own schedule and so shrinks as the activities spread across more resources while the tick count stays roughly the same:
...
- **P4. Redundant full-graph clones.** `CalculateResourceSchedulesByPriorityList` clones the builder (`VertexGraphBuilder.cs:934`) and then clones *that clone again* for the priority list (`:938-939`); each `CloneObject` -> `ToGraph` cleans edges and deep-copies every edge and node. The compiler additionally deep-clones all activities 2-3x per `Compile` (`VertexGraphCompiler.cs:250,283,308`). Fix: remove the double clone; reuse one working copy.
  ...
  **REJECTED** for the double clone (load-bearing - see the reassessment above), and the "2-3x per `Compile`" is a miscount: those branches are mutually exclusive, so it is one clone per compile. The two adjacent `Activities.ToList()` materialisations near the end of `Compile` **are** now shared, which is the part of this that was real.
...
- **P6. Recomputing property getters** - `ActivityIds`, `StartTime`, `FinishTime`, `Activities` are LINQ-over-`Values` each access; `ActivityIds.Contains(id)` (`:1096`) is an O(A) scan when `m_State.ContainsNode` is O(1). Use the dictionary directly; materialise once where enumerated repeatedly. **DONE**, except `ActivityIds`, where materialising would allocate more than the iterator it replaced. Worth a fraction of a percent; see the status entry above.
- **P7. `ResourceScheduleBuilder.ScheduledActivities` copies every access** (`ResourceScheduleBuilder.cs:63`, `.ToList()`), and the scheduler calls it repeatedly via `.Any()`/`.Select()`. Expose a non-copying enumerable/count. **DONE, but the premise was wrong** - no call site was ever in the tick loop, and the fix was not the non-copying enumerable this asks for but a `FirstActivityStartTime` property giving the scheduler the one number it actually wanted.
