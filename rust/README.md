...
- **Dependency injection** — the C# engine interfaces (SCC finder, CPM engine, transitive reducer factory, resource-scheduling engine, generators) are plain modules here; the engines-bundle constructors have no counterpart. ID generation keeps its seam as the `IdGenerator` enum.
