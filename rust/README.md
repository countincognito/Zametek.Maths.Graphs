...
- **Determinism** - insertion-ordered maps/sets stand in for the C# `Dictionary`/`HashSet`, whose iteration order the original's outputs depend on (schedule tie-breaking, error-message ordering, Tarjan component order). The graph states use a bespoke `InsertionOrderMap` (an insertion-ordered entry list plus a hash index, with tombstoned removals - structurally what the C# `Dictionary` does internally) so that removing an entry is O(1) amortised, matching `Dictionary.Remove`, while iteration order stays deterministic; `indexmap` covers the remaining maps and sets. The `shuffle_processing_order` hook is kept so tests can prove the CPM passes are order-independent.

One test goes further than copying values: the priority-list corpus (`compilers/tests/priority_list_corpus/`) is a value-for-value port of the C# generator, right down to its xorshift arithmetic, so both languages build the same 56 graphs - and the Rust output is checked against the **C# baseline file itself**, not just a local copy of it. The priority list decides the order activities are offered resources, and therefore the whole schedule, so that one comparison pins the two implementations together where it matters most.

The test suites port the C# tests **with the expected values copied verbatim**: every C# test file has a Rust counterpart, and every portable test is ported - the canonical nine-activity network's schedules and CPM values (including every constraint variant), the dummy-edge structural expectations, the full cost/billing/effort allocation matrices, resource dependencies and successors, and the exact compilation-error message strings. The deliberate omissions, documented per file, are the null-argument guards the type system makes unrepresentable, reference-identity/`GetHashCode` assertions (value semantics here), two JSON-fixture-driven tests (no serde dependency), the two `Guid` key cases (`Key` is integer-only), the cancellation-token tests (no counterpart in the port), and the corrupted-`HashSet` repros, whose input safe Rust cannot construct.
...
- **Cancellation** - the C# library requires a `CancellationToken` on every entry point whose cost grows with the graph. The port has none. There is no natural equivalent (the Rust idiom would be an `AtomicBool` or a callback), no consumer here needs one, and transliterating the C# shape would produce a parameter threaded through the whole API for nobody. This is a difference in surface, not in behaviour: an uncancelled compile takes the same path and produces the same answer in both. If it is ever wanted, it should be designed as Rust rather than ported.
- **`P0070`** - the input self-consistency probe is absent, and deliberately. It detects a set whose lookups disagree with its own contents, which arises from unsynchronized concurrent mutation - a state safe Rust cannot produce. Porting it would mean writing a check for a condition that cannot occur, which no test could exercise.
