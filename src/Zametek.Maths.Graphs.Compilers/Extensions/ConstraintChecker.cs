...
namespace Zametek.Maths.Graphs
{
        ...
        where TWorkStreamId : struct, IComparable<TWorkStreamId>, IEquatable<TWorkStreamId>
    {
        ...
        {
            if (!activity.EarliestStartTime.HasValue || !activity.EarliestFinishTime.HasValue) return;
        ...
        private static void CheckLateTimes(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.LatestStartTime.HasValue || !activity.LatestFinishTime.HasValue) return;
        ...
        private static void CheckStartTimeOrder(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.EarliestStartTime.HasValue || !activity.LatestStartTime.HasValue) return;
        ...
        private static void CheckFinishTimeOrder(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.EarliestFinishTime.HasValue || !activity.LatestFinishTime.HasValue) return;
        ...
        private static void CheckEarliestStartConstraint(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.EarliestStartTime.HasValue || !activity.MinimumEarliestStartTime.HasValue) return;
        ...
        private static void CheckLatestFinishConstraint(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.LatestFinishTime.HasValue || !activity.MaximumLatestFinishTime.HasValue) return;
        ...
        private static void CheckFreeSlackConstraint(
            ...
            IList<IInvalidConstraint<T>> output)
        {
            if (!activity.FreeSlack.HasValue || !activity.MinimumFreeSlack.HasValue) return;
